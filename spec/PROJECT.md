...
- **Prompt Size Limits**: Global configuration defines positive character limits for the student answer and cumulative unit test output, with optional per-exercise overrides. An oversized student answer is executed and tested but must not trigger an LLM request; a localized error is emitted. Oversized unit test output is truncated in configured test order before display or prompt construction; a localized warning is emitted while test names and pass/fail statuses are retained.
