...
def build_input_snapshot(config: ExerciseConfig, solution: StudentSolution) -> InputSnapshot:
    ...
    exercise_hash = compute_exercise_hash(config)
...
def is_stale(
    record: ExecutionRecord, live_config: ExerciseConfig, live_solution: StudentSolution
) -> bool:
    """Return True if the live exercise or solution has drifted from the record's snapshot."""
    live_exercise_hash = compute_exercise_hash(live_config)
